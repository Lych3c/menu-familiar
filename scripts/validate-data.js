...
['data/base.js','data/v3-init.js','data/v3-breakfasts.js','data/v3-lunches.js','data/v3-dinners.js','data/v4-library.js','data/v4-fixes.js','data/international-recipes.js','data/world-expansion-2.js','data/world-expansion-3.js','data/components.js','data/culinary-extensions.js','data/component-expansion-v12.js'].forEach(f=>vm.runInThisContext(fs.readFileSync(f,'utf8'),{filename:f}));
...
if(D.recipes.length!==462)fail(`Expected 462 recipes, got ${D.recipes.length}`);
...
if(library.length!==372)fail(`Expected 372 library recipes, got ${library.length}`);
const totals={Desayuno:144,Almuerzo:174,Cena:144};
...
if(new Set(D.recipes.map(r=>r.id)).size!==462)fail('Duplicate recipe IDs');
...
function carbFamily(name){const n=String(name).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(n.includes('papa'))return'papa';if(n.includes('arroz'))return'arroz';if(n.includes('yuca'))return'yuca';if(n.includes('platano'))return'platano';if(n.includes('guineo'))return'guineo';if(n.includes('arepa'))return'arepa';if(n.includes('mazorca')||n.includes('maiz')||n.includes('tortilla')||n.includes('polenta'))return'maiz';if(n.includes('avena'))return'avena';if(n.includes('quinua')||n.includes('quinoa'))return'quinua';if(n.includes('pasta')||n.includes('fideo')||n.includes('soba')||n.includes('udon'))return'pasta';if(n.includes('pan'))return'pan';if(n.includes('batata')||n.includes('camote'))return'batata';if(n.includes('harina integral'))return'harina';if(n.includes('dumpling'))return'dumpling';if(n.includes('pastel de arroz'))return'arroz';if(n.includes('roti'))return'roti';if(n.includes('pita'))return'pita';if(n.includes('semola'))return'semola';if(n.includes('masa filo'))return'filo';return n}
...
if(world.length!==162)fail(`Expected 162 international recipes, got ${world.length}`);
const countries=['Italia','Japón','México','Tailandia','Corea del Sur','Perú','India','España','Grecia'];
for(const country of countries){const rows=world.filter(r=>r.cuisine===country);if(rows.length!==18)fail(`${country}: expected 18 recipes, got ${rows.length}`);for(const t of ['Desayuno','Almuerzo','Cena']){const n=rows.filter(r=>r.type===t).length;if(n!==6)fail(`${country} ${t}: expected 6, got ${n}`)}}
for(const r of world){if(!r.origin||!r.cuisine||!r.chefTip)fail(`${r.id}: missing international provenance/technique metadata`);if(String(r.steps).length<140)fail(`${r.id}: international preparation too short (${String(r.steps).length})`)}
if(!world.some(r=>r.cuisine==='Tailandia'&&/pad thai/i.test(r.name)))fail('Thai library must include Pad Thai');
...
const mins={proteins:20,salads:40,cookedVegetables:40,carbs:14,sauces:50,soups:30};const componentIds=[];
...
for(const x of [...C.salads,...C.cookedVegetables,...C.sauces]){if(!Array.isArray(x.steps)||x.steps.length<3)fail(`${x.id}: needs detailed preparation steps`)}
...
console.log(`OK: ${D.recipes.length} recipes (${scheduled.length} scheduled + ${library.length} library), ${world.length} international recipes across ${countries.length} cuisines, ${lentil} lentil recipes, ${shopping.size} scheduled shopping items; components: ${C.proteins.length} proteins, ${C.salads.length} salads, ${C.cookedVegetables.length} cooked vegetables, ${C.carbs.length} carbs, ${C.sauces.length} sauces, ${C.soups.length} soups.`);
