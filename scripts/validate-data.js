...
['data/base.js','data/v3-init.js','data/v3-breakfasts.js','data/v3-lunches.js','data/v3-dinners.js','data/v4-library.js','data/v4-fixes.js','data/international-recipes.js','data/world-expansion-2.js','data/components.js','data/culinary-extensions.js'].forEach(f=>vm.runInThisContext(fs.readFileSync(f,'utf8'),{filename:f}));
...
if(D.recipes.length!==408)fail(`Expected 408 recipes, got ${D.recipes.length}`);
...
if(library.length!==318)fail(`Expected 318 library recipes, got ${library.length}`);
const totals={Desayuno:126,Almuerzo:156,Cena:126};
...
if(new Set(D.recipes.map(r=>r.id)).size!==408)fail('Duplicate recipe IDs');
...
if(world.length!==108)fail(`Expected 108 international recipes, got ${world.length}`);
const countries=['Italia','Japón','México','Tailandia','Corea del Sur','Perú'];
