...
const aliases={
 ...
 ensalada:['ensalada'],verdura:['verdura','vegetal'],brocoli:['brócoli','brocoli'],habichuela:['habichuela'],berenjena:['berenjena'],calabacin:['calabacín','calabacin']
};
...
function optionalList(group,type,limit=3,pairFamilies=[]){return (group||[]).map(x=>{const s=compScore(x,type),pairs=(x.pairs||[]).map(canonical),bonus=pairFamilies.some(f=>pairs.includes(canonical(f)))?3:0;return{...s,pairBonus:bonus}}).sort((a,b)=>b.pairBonus-a.pairBonus||b.uses-a.uses||a.missing.length-b.missing.length||b.coverage-a.coverage||a.time-b.time).slice(0,limit)}
...
function catalogCard(item,kind){const uses=kind==='sauce'?`<div class="catalog-uses"><b>Ideal para:</b> ${(item.uses||[]).map(esc).join(', ')}</div>`:`<div class="catalog-uses"><b>${esc(item.mealRole||'Sopa')}:</b> ${esc(item.note||'')}</div>`;return `<article class="catalog-item"><div class="catalog-title"><b>${esc(item.name)}</b><span>~${item.time||'—'} min</span></div>${uses}<div class="sub">Ingredientes: ${(item.ingredients||[]).map(esc).join(', ')}</div></article>`}
let lastPlates=[];
function append(){const root=document.querySelector('#pantry');if(!root)return;const picks=rankedPlates();lastPlates=picks.map(x=>x[1]);const best=picks[0]?.[1],pairFamilies=best?[canonical(best.p.item.name),canonical(best.c.item.name)]:[];const sauces=optionalList(C.sauces,'sauce',3,pairFamilies),soups=optionalList(C.soups,'soup',3);
 root.insertAdjacentHTML('beforeend',`<section class="auto-builder card"><div class="section-title"><div><h2>🧠 Constructor automático</h2><p>Combina proteína, vegetal y carbohidrato usando primero lo que ya tienes. Las propuestas son locales y no consumen IA.</p></div></div>${pantry().length?`<div class="auto-plates">${picks.map((x,i)=>plateCard(x[0],x[1],x[2],i)).join('')}</div>`:'<div class="empty compact-empty">Registra algunos ingredientes para generar propuestas automáticas.</div>'}<div class="optional-grid"><div><h3>🥣 Sopas y cremas sugeridas</h3>${soups.map(x=>miniCard(x,'soup')).join('')}</div><div><h3>🥗 Salsas y aderezos sugeridos</h3>${sauces.map(x=>miniCard(x,'sauce')).join('')}</div></div><div class="notice" style="margin-top:12px">Las sopas marcadas como <b>plato completo</b> pueden sustituir el plato principal. Si una sopa contiene papa, yuca, maíz, plátano o leguminosas, la app la señala para evitar duplicar carbohidratos innecesariamente.</div><details class="culinary-catalog"><summary>Ver catálogo completo de ${C.soups?.length||0} sopas y cremas</summary><div class="catalog-grid">${(C.soups||[]).map(x=>catalogCard(x,'soup')).join('')}</div></details><details class="culinary-catalog"><summary>Ver catálogo completo de ${C.sauces?.length||0} salsas y aderezos</summary><div class="catalog-grid">${(C.sauces||[]).map(x=>catalogCard(x,'sauce')).join('')}</div></details></section>`);
